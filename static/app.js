const mockData = [
  ...
  const populateProducts =() => {
    const products = document.querySelector('#products') // buscar id products
    products.innerHTML = ''
    for (const product of mockData) {
      const item = document.createElement('product-item')
      for (const key of ['name', 'rrp', 'info']) {
        const span = document.createElement('span') // crear elemento span con toda la informacion de key
        span.slot = key
